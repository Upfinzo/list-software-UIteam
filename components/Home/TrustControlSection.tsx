        ...
        <div className="mt-12 overflow-hidden rounded-2xl border border-white/10 sm:mt-14">
          <ul className="-mr-px -mb-px grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {trustControls.map((item) => (
              <li
                key={item.id}
                className="group relative border-r border-b border-white/10 p-6 transition-colors duration-300 hover:bg-white/[0.06] lg:p-7"
              >
                ...
                <p className="mt-2.5 text-[13px] leading-6 text-white/65 transition-colors duration-300 group-hover:text-white/80">
                  {item.description}
                </p>
              </li>
            ))}
